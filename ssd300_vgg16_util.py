...
weights_path = '/Volumes/Backup Plus/ds/models/VGG_VOC0712_SSD_300x300_iter_120000.h5'
IMAGE_PATH = '/Volumes/Backup Plus/ds/autonomous_cars/KITTI/'
...
def get_pred_uncertainty(fname, model, mc_dropout_size=20, 
    ...
    ground_truth_file = fname.replace('.png','.txt')
...
def find_uncertainties_for_all(files, mc_dropout=True):
    ...
    for p_size in p_size_list:
        ...
        rand_idx = np.random.randint(0,len(files),size=5000)
        avg_surface_list = []
        image_name_list = []
        for i in tqdm(range(len(rand_idx))):
            ...
            mc_location_file = 'tmp/ssd300/' + str(p_size) + '/kitti/' + ntpath.basename(fname).replace('.png','.npz')
            np.savez(mc_location_file,mc_locations)
        
        unc_df = pd.DataFrame({'image_name':image_name_list,'avg_surface':avg_surface_list})
        unc_df.to_csv('kitti_ssd300.csv', index=False)
    
def create_df_from_folder():
    npz_list = glob.glob('tmp/ssd300/kitti/*.npz')
    avg_uncs_list = []
    png_file_list = []
    for npz_path in npz_list:
        ...
        png_file = IMAGE_PATH + 'data_object_image_2/training/image_2/' + ntpath.basename(npz_path).replace('.npz','.png')
        png_file_list.append(png_file)
    df = pd.DataFrame({'image_name':png_file_list,'avg_surface':avg_uncs_list})
    df.to_csv('kitti_yolo.csv', index=False)
    return df
...
def plot_highly_uncertain_imgs(n=1, from_folder=False):
    if from_folder:
        ...
        unc_df = pd.read_csv('kitti_ssd300.csv')
    ...
    for index, row in unc_df.iterrows():
        ...
        mc_locations = np.load('tmp/ssd300/kitti/' + npz_file)['arr_0']
...
if __name__ == "__main__":
    files = glob.glob(IMAGE_PATH + 'data_object_image_2/training/image_2/*.png')
    find_uncertainties_for_all(files, mc_dropout=False)

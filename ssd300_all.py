...
def find_uncertainties_for_all(files, mc_dropout=True):
    ...
    for p_size in p_size_list:
        ...
        rand_idx = np.random.randint(0,len(files),size=2)
        avg_surface_list = []
        image_name_list = []
        for i in tqdm(range(len(rand_idx))):
            ...
            mc_location_file = fname + '_ssd300_' + str(mc_dropout_size) + '_' + str(p_size) +  '.npz'
            np.savez(mc_location_file,mc_locations)
        
        unc_df = pd.DataFrame({'image_name':image_name_list,'avg_surface':avg_surface_list})
        unc_df.to_csv('ssd300_' + IMAGE_NAME + '.csv', index=False)
    
def create_df_from_folder():
    npz_list = glob.glob('tmp/ssd300/' + IMAGE_NAME + '/*.npz')
    avg_uncs_list = []
    png_file_list = []
    ...
    df.to_csv('ssd300_' + IMAGE_NAME + '.csv', index=False)
    return df
...
def plot_highly_uncertain_imgs(n=1, from_folder=False):
    if from_folder:
        ...
        unc_df = pd.read_csv('ssd300' + IMAGE_NAME + '.csv')
    ...
    for index, row in unc_df.iterrows():
        ...
        mc_locations = np.load('tmp/ssd300/' + IMAGE_NAME + '/' + npz_file)['arr_0']
